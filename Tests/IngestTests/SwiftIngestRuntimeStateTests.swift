...
@Suite("SwiftIngestRuntimeState")
struct SwiftIngestRuntimeStateTests {
    ...
    func stateStoreRoundTrip() throws {
        ...
        let state = SwiftIngestRuntimeState(
        ...
        try SwiftIngestRuntimeStateStore.write(state, to: stateURL)
        let loaded = try SwiftIngestRuntimeStateStore.read(from: stateURL)
